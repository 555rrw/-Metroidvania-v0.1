...
| Room2 chapter flow | `src/world/Room2.tscn`, `src/tests/Room2FlowRunner.tscn` | GPT5.5 repaired second chapter flow: safe entry -> dash reward -> moving platform gap -> unstable platform pressure -> switch opens exit door -> dash gate/exit. | `src/tests/Room2FlowRunner.tscn`, `RouteRunner`, `DanielFeatureRunner`, Room2 screenshot capture. |
| Room4 Spirit Sanctum route | `src/world/Room4.tscn`, `src/tests/PostChapterFlowRunner.tscn` | GPT5.5 rebuilt later chapter from DanielDFY patterns: wide room, Vengeful Spirit reward, spell-only practice wall, moving platform, unstable platforms, saw/spikes, switch door, falling trap, gunner, soul cache. | `src/tests/PostChapterFlowRunner.tscn`, Room4 screenshot capture. |
| Room5 shortcut gauntlet route | `src/world/Room5.tscn`, `src/tests/PostChapterFlowRunner.tscn` | GPT5.5 rebuilt shortcut chapter from DanielDFY patterns: spell gate, spell-only wall, moving platform, unstable platforms, saw/spikes, switch door, falling trap, gunner, secret soul pickup, Room1 shortcut return. | `src/tests/PostChapterFlowRunner.tscn`, Room5 screenshot capture. |
